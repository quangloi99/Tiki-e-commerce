...
headers = {
    ...
    'Referer': 'https://tiki.vn/am-dun-sieu-toc-inox-2-lop-sunhouse-shd1351-1-8-lit-hang-chinh-hang-p102432143.html?itm_campaign=CTP_YPD_TKA_PLA_UNK_ALL_UNK_UNK_UNK_UNK_X.67936_Y.592502_Z.1249586_CN.Product-Ads_SA&itm_medium=CPC&itm_source=tiki-ads&spid=102432144',
...
df_id = pd.read_csv(f'product_id_diengd({datetime.now().strftime("%d-%m-%Y")}).csv')
p_ids = list(df_id.id)
print(p_ids)
result = []
...
df_product['Date'] = datetime.today().strftime('%Y-%m-%d')
df_product.to_csv(f'crawled_data_diengd({datetime.now().strftime("%d-%m-%Y")}).csv', index=False)
